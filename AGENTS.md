...
Packages/
  ...
  PhotoDescribeKit/       # feature kit: photo → short caption via VLM (SwiftPM; DEBUG)
Apps/
  ...
  PhotoDescribeDebug/     # DEBUG-only harness app running PhotoDescribeKit alone
...
- **PhotoDescribeKit** (`Packages/PhotoDescribeKit/`): `PhotoDescribing`, `DevicePhotoDescriber` / `MLXPhotoDescriber` (VLM path on the same Qwen pack), `PhotoDescribeFeatureView`. Depends on StoryPromptKit for the shared model store. Not in the host yet.
## Technology stack
...
- One Xcode project (`Vovozinha.xcodeproj`) with shared schemes in `xcshareddata/xcschemes`: **Vovozinha** (host), **StoryPromptDebug** / **PhotoDescribeDebug** (kit harnesses), **VovozinhaLegacy** (old monolith). Package schemes are auto-generated by Xcode for unit tests.
- Static text is **Markdown on disk** — edit and rebuild, no code changes:
  ...
  - Photo VLM prompts: `Packages/PhotoDescribeKit/Sources/PhotoDescribeKit/Resources/Prompts/describe.<lang>.md`
...
xcodebuild -scheme PhotoDescribeDebug -destination 'generic/platform=iOS' build

# StoryPromptKit / PhotoDescribeKit unit tests (macOS package tests; mocks only — no real model)
...
- **PhotoDescribeKitTests**: prompt load, caption normalize, mock VLM session, `DevicePhotoDescriber` throws without a model. Live VLM only on device.
